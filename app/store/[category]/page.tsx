  ...
  return (
    <div data-aos="fade-up" className="flex flex-col items-center min-h-screen p-8 sm:p-15 font-[family-name:var(--font-poppins)]">
      <div className="grid items-center grid-cols-1 lg:grid-cols-3 2xl:grid-cols-4 gap-x-8 gap-y-4 mt-30">
        ...
        : 
        products.sort((a, b) => (b.isNew ? 1 : 0) - (a.isNew ? 1 : 0)).map((value, index) => (
            <Link
              key={index}
            ...
            >
              <div className="flex items-center justify-center w-full h-[240px] text-white bg-white/4 border border-white/3 backdrop-blur-md overflow-hidden mt-auto p-4 gap-2 rounded-xl font-prompt relative">
                <Image src={value.image ? value.image : "/product.png"} fill alt="product" className="object-cover" />
                {value.isNew && (
                  <div className="z-10 bg-[#cd0101]/60 p-2 ml-auto mb-auto rounded-md">
                    New
                  </div>
                )}
              </div>
